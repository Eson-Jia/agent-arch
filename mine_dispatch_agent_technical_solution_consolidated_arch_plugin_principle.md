...
0. **智能调度 Agent 必须是完全独立的旁路插件，原有调度系统不能对 Agent 产生任何运行时依赖。Agent 故障、降级、关闭或整体摘除后，生产调度、车辆运行、任务派发、告警处理等原有生产运营链路必须继续正常运行。**
1. 不侵入现有调度核心链路。
2. 不让 LLM 直接调用 Dubbo。
...
8. LLM Proxy 或模型供应商故障时，只影响智能问答和智能分析，不影响生产调度。
...
现有调度系统
  -> 不依赖 mine-agent-app
  -> 不依赖 operator-mcp
  -> 不依赖 llm-proxy-router
```

这意味着 operator-mcp 是 Agent 到生产能力的受控出口，不是现有调度系统的新基础设施。
